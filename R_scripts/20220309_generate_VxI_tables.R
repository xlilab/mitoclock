...
#   dir(str_c("~/data/pipeline/mitochondria_m2_RNA_wdl/GTEx_v8_new/",x,"/output_vcf/"),pattern = "consensus.vcf$",full.names = T)
# })
# files <- do.call(c,files)
# 
# data_list <- lapply(files,process_single_vcf)
# data <- do.call(rbind,data_list)
# write_tsv(data,"~/data/scientific_project/mitochondrial_genome/tables/20230529_Vmtrna_Igtex_T_consensus.txt")
...
#            log2()) %>%
#   ungroup()
# fwrite(Vmtrna_Igtex_T_raw,"~/data/scientific_project/mitochondrial_genome/tables/20230530_Vmtrna_Igtex_T_consensus_fisher.txt",quote = F,sep = "\t")
...
# }
# 
# Vmtrna_Igtex_T_raw <- fread("~/data/scientific_project/mitochondrial_genome/tables/20230530_Vmtrna_Igtex_T_consensus_fisher.txt")
# Vmtrna_Igtex_T_raw <- calc_sequencing_error_p(Vmtrna_Igtex_T_raw)
# fwrite(Vmtrna_Igtex_T_raw,"~/data/scientific_project/mitochondrial_genome/tables/20230413_Vmtrna_Igtex_T_original_fisher_binomp.txt",quote = F,sep = "\t")
...
# process_single_vcf <- function(file){
#   data <- read.vcfR(file)
#   data <- vcfR2tidy(data,single_frame = T,format_fields = c("AD","AF","DP")) %>% 
#     .$dat %>% 
#     select(CHROM:ALT,FILTER,Indiv:gt_DP) %>% 
#     mutate(ID = str_c(CHROM,POS,REF,ALT,sep = "_"),
#            gt_AD_REF = str_split(gt_AD,pattern = ",",simplify = T)[,1],
#            gt_AD_ALT = str_split(gt_AD,pattern = ",",simplify = T)[,2])
#   return(data)
# }
# 
# dirs <- c("GTEx_v8_whole_blood","GTEx_v8_other_tissues")
# types <- c("contamination","low_coverage","output")
# dirs <- lapply(dirs,function(x) str_c(x,types,sep = "/")) %>% 
#   unlist()
# files <- lapply(dirs,function(x){
#   dir(str_c("~/data/pipeline/mitochondria_m2_wdl/",x),pattern = "final.split.vcf$",full.names = T)
# }) %>% 
#   unlist()
# 
# data_list <- lapply(files,process_single_vcf)
# data <- do.call(rbind,data_list)
# 
# sample_info <- read_tsv("~/data/datasets/gtex/v8/annotations/GTEx_Analysis_2017-06-05_v8_Annotations_SampleAttributesDS.txt")
# sample_info <- sample_info %>% 
#   filter(SMAFRZE == "WGS") %>% 
#   mutate(Indiv = str_extract(SAMPID,"GTEX-\\w+-\\d+"),
#          tissue = str_replace_all(SMTSD,"[ |)]","")) %>% 
#   mutate(tissue = str_replace_all(tissue,"\\(","_")) %>% 
#   select(Indiv,tissue)
# 
# data <- data %>% 
#   left_join(sample_info) %>% 
#   mutate(Indiv = str_extract(Indiv,"GTEX-\\w+")) %>% 
#   arrange(tissue,Indiv)
# data$tissue[is.na(data$tissue)] <- "WholeBlood"
# write_tsv(data,"~/data/scientific_project/mitochondrial_genome/tables/20220314_Vmtdna_Igtex_T.txt")


# # TOPMed mitochondrial tables
# # Vmtdna_Itopmed ----------------------------------------------------------
...
# files <- dir("~/data/pipeline/mitochondria_m2_wdl/TOPMed/output/",pattern = "final.split.vcf$",full.names = T)

# data_list <- lapply(files,process_single_vcf)
# data <- do.call(rbind,data_list)

# write_tsv(data,"~/data/scientific_project/mitochondrial_genome/tables/20220730_Vmtdna_Itopmed.txt")
